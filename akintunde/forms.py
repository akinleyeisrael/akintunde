...
from .models import About, Gallery, Category, Blog, Contact,Video, Exhibition
...
class ExhibitionFomr(forms.ModelForm):
    class Meta:
        model = Exhibition
        fields = "__all__"
        exclude = ["description", "video_file"]
        widgets = {
            'title': forms.TextInput(attrs={'class':'form-control', 'placeholder' : 'Title- tell about your exhibition'}),
            'description': forms.Textarea(attrs={'class':'form-control', 'placeholder' : 'Description'}),
             'image': forms.FileInput(attrs={'class':'form-control', 'placeholder' : 'Image'}),
        }